...
class DeveloperLayoutMici(NavScroller):
  def __init__(self):
    ...
    self._v2i_toggle = BigToggle("vehicle-to-infrastructure",
                                 initial_state=ui_state.params.get_bool("V2IEnabled"),
                                 toggle_callback=self._on_v2i_enabled)
    self._phone_nav_toggle = BigToggle("iphone navigation (carplay)",
                                       initial_state=ui_state.params.get_bool("PhoneNavEnabled"),
                                       toggle_callback=self._on_phone_nav_enabled)
    self._att_toggle = BigToggle("at&t data plan + wifi",
                                 initial_state=ui_state.params.get_bool("AttDataPlanEnabled"),
                                 toggle_callback=self._on_att_dataplan_enabled)
    self._tmobile_toggle = BigToggle("t-mobile data plan + wifi",
                                     initial_state=ui_state.params.get_bool("TmobileDataPlanEnabled"),
                                     toggle_callback=self._on_tmobile_dataplan_enabled)
    self._verizon_toggle = BigToggle("verizon data plan + wifi",
                                     initial_state=ui_state.params.get_bool("VerizonDataPlanEnabled"),
                                     toggle_callback=self._on_verizon_dataplan_enabled)
    ...
    self._scroller.add_widgets([
      ...
      self._att_toggle,
      self._tmobile_toggle,
      self._verizon_toggle,
    ])
    ...
    self._refresh_toggles = (
      ...
      ("AttDataPlanEnabled", self._att_toggle),
      ("TmobileDataPlanEnabled", self._tmobile_toggle),
      ("VerizonDataPlanEnabled", self._verizon_toggle),
    )
    onroad_blocked_toggles = (self._adb_toggle, self._joystick_toggle, self._v2i_toggle, self._phone_nav_toggle, self._att_toggle, self._tmobile_toggle, self._verizon_toggle)
  ...
  def _on_v2i_enabled(self, state: bool):
    ui_state.params.put_bool("V2IEnabled", state)
    restart_needed_callback(state)

  def _on_phone_nav_enabled(self, state: bool):
    ui_state.params.put_bool("PhoneNavEnabled", state)
    restart_needed_callback(state)

  def _on_att_dataplan_enabled(self, state: bool):
    ui_state.params.put_bool("AttDataPlanEnabled", state)
    if state:
      ui_state.params.put_bool("TmobileDataPlanEnabled", False)
      ui_state.params.put_bool("VerizonDataPlanEnabled", False)
      ui_state.params.put("V2ICarrier", "att")
      ui_state.params.put_bool("AttWifiHotspot", True)
      ui_state.params.put_bool("PhoneNavEnabled", True)
      ui_state.params.put_bool("V2IEnabled", True)
    restart_needed_callback(state)

  def _on_tmobile_dataplan_enabled(self, state: bool):
    ui_state.params.put_bool("TmobileDataPlanEnabled", state)
    if state:
      ui_state.params.put_bool("AttDataPlanEnabled", False)
      ui_state.params.put_bool("VerizonDataPlanEnabled", False)
      ui_state.params.put("V2ICarrier", "tmobile")
      ui_state.params.put_bool("TmobileWifiHotspot", True)
      if not ui_state.params.get("V2ITmobileApn"):
        ui_state.params.put("V2ITmobileApn", "fast.t-mobile.com")
      ui_state.params.put_bool("PhoneNavEnabled", True)
      ui_state.params.put_bool("V2IEnabled", True)
    restart_needed_callback(state)

  def _on_verizon_dataplan_enabled(self, state: bool):
    ui_state.params.put_bool("VerizonDataPlanEnabled", state)
    if state:
      ui_state.params.put_bool("AttDataPlanEnabled", False)
      ui_state.params.put_bool("TmobileDataPlanEnabled", False)
      ui_state.params.put("V2ICarrier", "verizon")
      ui_state.params.put_bool("VerizonWifiHotspot", True)
      if not ui_state.params.get("V2IVerizonApn"):
        ui_state.params.put("V2IVerizonApn", "vzwinternet")
      ui_state.params.put_bool("PhoneNavEnabled", True)
      ui_state.params.put_bool("V2IEnabled", True)
    restart_needed_callback(state)
